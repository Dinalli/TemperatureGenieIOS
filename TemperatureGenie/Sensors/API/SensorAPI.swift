...
struct SensorAPI {
    ...
    func getSensorForID(token: String, sensorId: String, session: URLSession) -> AnyPublisher<UserSensorResponse, APIError> {
        let urlString = "\(apiPath)/api/Temperature/Get/\(sensorId)"
        guard let downloadURL: URL = URL.init(string: urlString) else {
            return Fail(error: .errorDescription("")).eraseToAnyPublisher()
        }
        var urlRequest: URLRequest = URLRequest.init(url: downloadURL)
            ...
            .flatMap { data, response -> AnyPublisher<UserSensorResponse, APIError> in
                ...
                if (200...299).contains(response.statusCode) {
                    ...
                    do {
                        // if decoding fails, decode as an `ErrorResponse`
                        let error = try jsonDecoder.decode(LoginErrorResponseObject.self, from: data)
                        return Fail(error: APIError.errorDescription(error.errors.arrayStrings.first ?? "Sorry we could not get any sensor for the sensor alert"))
                            .eraseToAnyPublisher()
                    } catch {
                        // if both fail, throw APIError.decoding
                        return Fail(error: APIError.unknown)
                    ...
                    do {
                        // if decoding fails, decode as an `ErrorResponse`
                        let error = try jsonDecoder.decode(LoginErrorResponseObject.self, from: data)
                        return Fail(error: APIError.errorDescription(error.errors.arrayStrings.first ?? "Sorry we could not get any sensor for the sensor alert"))                            .eraseToAnyPublisher()
                    } catch {
                        // if both fail, throw APIError.decoding
                        return Fail(error: APIError.unknown)
                            .eraseToAnyPublisher()
                    ...
                    return Fail(error: .errorDescription("Sorry we could not get any sensor for the sensor alert"))
                        .eraseToAnyPublisher()
                }
            }.eraseToAnyPublisher()
    }
    ...
    func submitPauseForSenson(token: String, pauseSubmission: PauseAlarmSubmission, session: URLSession) -> AnyPublisher<BasicResponseObject, APIError> {
        let urlString = "\(apiPath)/api/Temperature/PauseAlarm"
        guard let downloadURL: URL = URL.init(string: urlString) else {
            return Fail(error: .errorDescription("")).eraseToAnyPublisher()
        }
        var urlRequest: URLRequest = URLRequest.init(url: downloadURL)
        ...
        let encoder = JSONEncoder()
        guard let httpBody = try? encoder.encode(pauseSubmission) else {
            return Fail(error: .errorDescription("")).eraseToAnyPublisher()
        }
        urlRequest.httpBody = httpBody
            ...
            .flatMap { data, response -> AnyPublisher<BasicResponseObject, APIError> in
                ...
                if (200...299).contains(response.statusCode) {
                    ...
                    do {
                        // if decoding fails, decode as an `ErrorResponse`
                        let error = try jsonDecoder.decode(ManualEntryErrorResponseObject.self, from: data)
                        return Fail(error: APIError.errorDescription(error.errors.arrayStrings.first ?? "Sorry pause submission failed. Please try again later"))                            .eraseToAnyPublisher()
                    } catch {
                        // if both fail, throw APIError.decoding
                        return Fail(error: APIError.unknown)
                            .eraseToAnyPublisher()
                    ...
                    return Fail(error: .errorDescription("Sorry pause submission failed. Please try again later"))
                        .eraseToAnyPublisher()
                }
            }.eraseToAnyPublisher()
    }
    
    func submitAlertAlarm(token: String, alertAlarmReadingSubmission: AlertAlarmReadingSubmission, session: URLSession) -> AnyPublisher<BasicResponseObject, APIError> {
        let urlString = "\(apiPath)/api/Temperature/ActionAlarm"
        guard let downloadURL: URL = URL.init(string: urlString) else {
            return Fail(error: .errorDescription("")).eraseToAnyPublisher()
        }
        var urlRequest: URLRequest = URLRequest.init(url: downloadURL)
        ...
        let encoder = JSONEncoder()
        guard let httpBody = try? encoder.encode(alertAlarmReadingSubmission) else {
            return Fail(error: .errorDescription("")).eraseToAnyPublisher()
        }
        urlRequest.httpBody = httpBody
            ...
            .flatMap { data, response -> AnyPublisher<BasicResponseObject, APIError> in
                ...
                if (200...299).contains(response.statusCode) {
                    ...
                    do {
                        // if decoding fails, decode as an `ErrorResponse`
                        let error = try jsonDecoder.decode(ManualEntryErrorResponseObject.self, from: data)
                        return Fail(error: APIError.errorDescription(error.errors.arrayStrings.first ?? "Sorry alert submission failed. Please try again later"))                            .eraseToAnyPublisher()
                    } catch {
                        // if both fail, throw APIError.decoding
                        return Fail(error: APIError.unknown)
                            .eraseToAnyPublisher()
                    ...
                    return Fail(error: .errorDescription("Sorry alert submission failed. Please try again later"))
                        .eraseToAnyPublisher()
                }
            }.eraseToAnyPublisher()
    }
